...
import type { SiteConfig, NavItem, Service, AppearanceFooter } from "@/lib/wordpress";
...
export default function Footer({ siteConfig, navigation, services, logo, footerSettings }: FooterProps) {
  ...
  const footerDescription = footerSettings?.description || "Expert en couverture, charpente et toiture dans le Var. Qualité, fiabilité et savoir-faire au service de votre habitation.";
  const copyright = footerSettings?.copyright || `© ${new Date().getFullYear()} COUVERTURE J.J.M. Tous droits réservés.`;
  ...
  const visibleColumns = [true, showServices, showLinks, showContact].filter(Boolean).length;
  const gridCols = visibleColumns <= 2 ? "lg:grid-cols-2" : visibleColumns === 3 ? "lg:grid-cols-3" : "lg:grid-cols-4";
    ...
    >
            ...
            <div className="mb-6">
              {footerLogo ? (
                <img
                  ...
                  style={{ height: `${footerLogoHeight}px`, maxHeight: `${footerLogoHeight}px` }}
                />
              ) : (
                <h3
                ...
                <li>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium break-all" style={{ color: headingColor }}>
                        {siteConfig.email}
                      </p>
                    </div>
                  </a>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-6 text-sm" style={{ color: bottomBarText }}>
            <Link
              href="/mentions-legales"
